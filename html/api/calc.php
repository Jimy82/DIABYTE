...
if (!csrf_check($data['csrf'] ?? '')) { http_response_code(400); echo json_encode(['error' => 'csrf']); exit; }

$foodId = (int)($data['food_id'] ?? 0);
$grams  = (float)($data['grams'] ?? 0);
$preBg  = isset($data['pre_bg']) ? (float)$data['pre_bg'] : null;

if ($foodId <= 0 || $grams <= 0) { http_response_code(422); echo json_encode(['error' => 'params']); exit; }
...
if (!$food) { http_response_code(404); echo json_encode(['error' => 'food']); exit; }

$carbs = round(((float)$food['carbs_per_100g'] * $grams) / 100.0, 2);

$st2 = $pdo->prepare('SELECT carb_ratio, insulin_sensitivity, target_bg FROM insulin_params WHERE user_id=? LIMIT 1');
$st2->execute([ (int)$_SESSION['user_id'] ]);
$params = $st2->fetch(PDO::FETCH_ASSOC);

$dose = null;
if ($params) {
  $ratio = (float)$params['carb_ratio'];           // g HC por 1U
  $isf   = (float)$params['insulin_sensitivity'];  // mg/dL por 1U
...
echo json_encode([
  'food'   => ['id' => (int)$food['id'], 'name' => $food['name'], 'unit' => $food['unit'], 'gi' => $food['glycemic_index']],
  'input'  => ['grams' => $grams, 'pre_bg' => $preBg],
  'result' => ['carbs_g' => $carbs, 'dose_units' => $dose]
]);
